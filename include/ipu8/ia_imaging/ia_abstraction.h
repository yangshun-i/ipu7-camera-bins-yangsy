 ...
 */
#define IA_ALIGN(a,b)               ((CAST_TO_TYPE(uint32_t, (a)) + CAST_TO_TYPE(uint32_t, (b-1U))) & (~(CAST_TO_TYPE(uint32_t, (b-1U)))))
...
#define IA_FREEZ(x)              { (void)free(x); x = NULL;}
...
#define IA_ROUND(a)              ((CAST_TO_TYPE(float32_t,(a)) > 0.0F) ? IA_FLOOR(CAST_TO_TYPE(float32_t,(a)) + 0.5F) : IA_CEIL(CAST_TO_TYPE(float32_t,(a)) - 0.5F))
#define IA_ROUNDD(a)             ((CAST_TO_TYPE(float64_t,(a)) > 0.0) ? IA_FLOORD(CAST_TO_TYPE(float64_t,(a)) + 0.5) : IA_CEILD((CAST_TO_TYPE(float64_t,a) - 0.5)))
#define IA_ROUNDF(a)             ((static_cast<float32_t>(a) > static_cast<float32_t>(0.0F)) ? IA_FLOOR(static_cast<float32_t>(a) + static_cast<float32_t>(0.5F)) : IA_CEIL(static_cast<float32_t>(a) - static_cast<float32_t>(0.5F)))
...
#define IA_LOG2(x)               (logf(CAST_TO_TYPE(float32_t,(x))) / logf(CAST_TO_TYPE(float32_t,(2.0F))))
...
//#define IA_MAX_FIXEDPOINT(integer_bits, frac_bits) IA_MAX_FIXEDPOINT(integer_bits, frac_bits)
...
/* S5.10 means: total 15 bits =  1 sign bit + 5 int bits + 10 fractional bits*/
#define IA_SX_10_FRAC_BITS  (10U)
#define IA_S5_10_MIN IA_MIN_FIXEDPOINT(5U, IA_SX_10_FRAC_BITS)
...
inline float64_t   IA_MAX_FIXEDPOINT(const int32_t integer_bits, const int32_t frac_bits)
{
    const uint32_t temp_num = ((((integer_bits) > 0) ? (CAST_TO_TYPE(uint32_t, 2U) << (CAST_TO_TYPE(uint32_t, integer_bits) - 1U)) : 1));
    float64_t res = 0.0;
    if (frac_bits > 0)
    {
      res = 1.0 / CAST_TO_TYPE(float64_t, (CAST_TO_TYPE(uint32_t, 2U) << (CAST_TO_TYPE(uint32_t, frac_bits) - 1U)));
    }
    res = (CAST_TO_TYPE(float64_t, temp_num)) - res;
    return res;
}

inline uint32_t   IA_FLOAT_TO_Q0_31(const float64_t val)
{
    const uint32_t IA_FLOAT_TO_Q0_31_temp_param = CAST_TO_TYPE(uint32_t, 2U) << (IA_QX_31_FRAC_BITS - 1U);
    const float64_t IA_FLOAT_TO_Q0_31_limit_res = (IA_LIMIT(val, IA_Q0_31_MIN, IA_Q0_31_MAX)) * (CAST_TO_TYPE(uint64_t, IA_FLOAT_TO_Q0_31_temp_param));
    const float64_t IA_FLOAT_TO_Q0_31_round_rs = IA_ROUNDD(IA_FLOAT_TO_Q0_31_limit_res);
    return CAST_TO_TYPE(uint32_t, IA_FLOAT_TO_Q0_31_round_rs);
}

inline uint32_t IA_FLOAT_TO_Q0_26(const float64_t val)
{
    const uint32_t IA_FLOAT_TO_Q0_26_temp_param = CAST_TO_TYPE(uint32_t, 2U) << (IA_QX_26_FRAC_BITS - 1U);
    const float64_t IA_FLOAT_TO_Q0_26_limit_res = IA_LIMIT(val, IA_Q0_26_MIN, IA_Q0_26_MAX) * (CAST_TO_TYPE(uint64_t, IA_FLOAT_TO_Q0_26_temp_param));
    const float32_t IA_FLOAT_TO_Q0_26_round_rs = IA_ROUND(IA_FLOAT_TO_Q0_26_limit_res);
    return CAST_TO_TYPE(uint32_t, IA_FLOAT_TO_Q0_26_round_rs);
}


inline uint32_t  IA_FLOAT_TO_Q14_16(const float64_t val)
{
    const uint32_t IA_FLOAT_TO_Q14_16_temp_param = CAST_TO_TYPE(uint32_t, 2U) << (IA_QX_16_FRAC_BITS - 1U);
    const float64_t IA_FLOAT_TO_Q14_16_limit_res = (IA_LIMIT(val, IA_Q14_16_MIN, IA_Q14_16_MAX)) * (CAST_TO_TYPE(uint64_t, IA_FLOAT_TO_Q14_16_temp_param));
    const float64_t IA_FLOAT_TO_Q14_16_round_rs = IA_ROUND(IA_FLOAT_TO_Q14_16_limit_res);
    return CAST_TO_TYPE(uint32_t, IA_FLOAT_TO_Q14_16_round_rs);
}
inline uint32_t IA_FLOAT_TO_Q3_16(const float64_t val)
{
    const uint32_t IA_FLOAT_TO_Q3_16_temp_param = CAST_TO_TYPE(uint32_t, 2U) << (IA_QX_16_FRAC_BITS - 1U);
    const float64_t IA_FLOAT_TO_Q3_16_limit_res = (IA_LIMIT(val, IA_Q3_16_MIN, IA_Q3_16_MAX)) * (CAST_TO_TYPE(uint64_t, IA_FLOAT_TO_Q3_16_temp_param));
    const float64_t IA_FLOAT_TO_Q3_16_round_rs = IA_ROUNDD(IA_FLOAT_TO_Q3_16_limit_res);
    return CAST_TO_TYPE(uint32_t, IA_FLOAT_TO_Q3_16_round_rs);
}


inline int32_t IA_FLOAT_TO_S4_15(const float32_t val)
{
    const uint32_t IA_FLOAT_TO_S4_15_temp_param = CAST_TO_TYPE(uint32_t, 2U) << (IA_SX_15_FRAC_BITS - 1U);
    const float64_t IA_FLOAT_TO_S4_15_limit_res = IA_LIMIT(val, IA_S4_15_MIN, IA_S4_15_MAX) * (CAST_TO_TYPE(uint64_t, IA_FLOAT_TO_S4_15_temp_param));
    const float32_t IA_FLOAT_TO_S4_15_round_rs = IA_ROUND(IA_FLOAT_TO_S4_15_limit_res);
    return CAST_TO_TYPE(int32_t, IA_FLOAT_TO_S4_15_round_rs);
}

inline int32_t IA_FLOAT_TO_S1_20(const float32_t val)
{
    const uint32_t IA_FLOAT_TO_S1_20_temp_param = CAST_TO_TYPE(uint32_t, 2U) << (IA_SX_20_FRAC_BITS - 1U);
    const float64_t IA_FLOAT_TO_S1_20_temp_limit_res = IA_LIMIT(val, IA_S1_20_MIN, IA_S1_20_MAX) * (CAST_TO_TYPE(uint64_t, IA_FLOAT_TO_S1_20_temp_param));
    const float32_t IA_FLOAT_TO_S1_20_temp_round_rs = IA_ROUND(IA_FLOAT_TO_S1_20_temp_limit_res);
    return CAST_TO_TYPE(int32_t, IA_FLOAT_TO_S1_20_temp_round_rs);
}

inline uint16_t IA_FLOAT_TO_S5_10(const float32_t val)
{
    const uint32_t IA_FLOAT_TO_S5_10_temp_param = CAST_TO_TYPE(uint32_t, 2U) << (IA_SX_10_FRAC_BITS - 1U);
    const float64_t IA_FLOAT_TO_S5_10_limit_res = (IA_LIMIT(CAST_TO_TYPE(float64_t, val), IA_S5_10_MIN, IA_S5_10_MAX)) * (CAST_TO_TYPE(uint64_t, IA_FLOAT_TO_S5_10_temp_param));
    const float64_t IA_FLOAT_TO_S5_10_round_rs = IA_ROUNDD(IA_FLOAT_TO_S5_10_limit_res);
    return CAST_TO_TYPE(uint16_t, IA_FLOAT_TO_S5_10_round_rs);
}

inline int32_t IA_FLOAT_TO_S4_20(const float64_t val)
{
    const uint32_t IA_FLOAT_TO_S4_20_temp_param = CAST_TO_TYPE(uint32_t, 2U) << (IA_SX_20_FRAC_BITS - 1U);
    const float64_t IA_FLOAT_TO_S4_20_limit_res = (IA_LIMIT(val, IA_S4_20_MIN, IA_S4_20_MAX)) * (CAST_TO_TYPE(uint64_t, IA_FLOAT_TO_S4_20_temp_param));
    const float64_t IA_FLOAT_TO_S4_20_round_rs = IA_ROUNDD(IA_FLOAT_TO_S4_20_limit_res);
    return CAST_TO_TYPE(int32_t, IA_FLOAT_TO_S4_20_round_rs);
}

inline int32_t IA_FLOAT_TO_S14_8(const float64_t val)
{
    const uint32_t IA_FLOAT_TO_S14_8_temp_param = CAST_TO_TYPE(uint32_t, 2U) << (IA_SX_8_FRAC_BITS - 1U);
    const float64_t IA_FLOAT_TO_S14_8_limit_res = (IA_LIMIT(val, IA_S14_8_MIN, IA_S14_8_MAX)) * (CAST_TO_TYPE(uint64_t, IA_FLOAT_TO_S14_8_temp_param));
    const float64_t IA_FLOAT_TO_S14_8_round_rs = IA_ROUNDD(IA_FLOAT_TO_S14_8_limit_res);
    return CAST_TO_TYPE(int32_t, IA_FLOAT_TO_S14_8_round_rs);
}


inline int32_t IA_FLOAT_TO_S18_8(const float64_t val)
{
    const uint32_t IA_FLOAT_TO_S18_8_temp_param = CAST_TO_TYPE(uint32_t, 2U) << (IA_SX_8_FRAC_BITS - 1U);
    const float64_t IA_FLOAT_TO_S18_8_limit_res = IA_LIMIT(val, IA_S18_8_MIN, IA_S18_8_MAX) * (CAST_TO_TYPE(uint64_t, IA_FLOAT_TO_S18_8_temp_param));
    const float64_t IA_FLOAT_TO_S18_8_round_rs = IA_ROUNDD(IA_FLOAT_TO_S18_8_limit_res);
    return CAST_TO_TYPE(int32_t, IA_FLOAT_TO_S18_8_round_rs);
}

#else

#define IA_MAX_FIXEDPOINT(integer_bits, frac_bits) (CAST_TO_TYPE(float64_t,((((integer_bits) > 0)?(2U<<(CAST_TO_TYPE(uint32_t,integer_bits)-1U)):1U))) - (1.0/(CAST_TO_TYPE(float64_t,(((frac_bits) > 0)?(2U<<(CAST_TO_TYPE(uint32_t,frac_bits)-1U)):0U)))))
